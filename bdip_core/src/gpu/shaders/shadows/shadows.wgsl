...
@group(1) @binding(0) var<uniform> params: ShadowsParams;
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // W_s=1 for pure shadows, 0 for mid-tones and above.
    let w_s = 1.0 - smoothstep(params.start, params.range, L);
    let out_rgb = color.rgb + (params.amt * w_s * color.rgb);
