// Tilt-Shift — box-filter downsample pass.
//
// Reduces the input to output dimensions using a box filter. The scale factor is
// derived from texture dimensions at dispatch time, so PassScale::Down(4) on the
// Rust side controls allocation; this shader adapts automatically.
//
// Declares the full TiltShiftParams struct to satisfy WebGPU's uniform
// binding-size validation (the engine validates the uniform binding size
// against the pipeline layout at creation time).
