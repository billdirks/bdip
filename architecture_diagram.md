...
The following diagram provides a top-down view of the strict module separation inside the `ImageProcessingApp` workspace. It illustrates the data boundary isolating the **Application Binary (UI & CLI routing)** from the **Core Processing Library (GPU execution & Data I/O)**.
...
flowchart TB
    ...
    subgraph AppBinary ["📦 Application Binary (app_bin)"]
    ...
    subgraph CoreLib ["📦 Core Processing Library (core_lib)"]
